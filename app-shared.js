...
(function(){
  ...
  api.installStudyTools = function(){
    ...
    const addQuestion=(q)=>{if(!q)return;const id=`${q.module||''}:${q.id||q.prompt||''}`;if(data.saved.some(x=>x.key===id))return;const prompt=q.prompt?new DOMParser().parseFromString(q.prompt,'text/html').body.textContent.trim():q.en||q.question||q.expectedDisplay||q.sp||(q.number!=null?`Number ${q.number}`:'Question');data.saved.push({key:id,qid:q.id,module:q.module,moduleName:document.querySelector('#activeModuleLabel')?.textContent||q.module,prompt,answer:q.expectedDisplay||q.sp||q.answer||'',number:q.number,mode:q.mode,options:q.options,correctIndex:q.correctIndex,acceptable:[...(q.acceptable||[])],explanation:q.explanation,selected:true});save(data);star.textContent='★ Saved';star.disabled=true;after.hidden=true;renderReview();};
    const star=document.createElement('button');star.type='button';star.className='study-question-star';star.textContent='☆ Save question';star.onclick=()=>{const app=window.SpanishPracticeApp;if(!app?.currentQuestion)return;addQuestion(app.currentQuestion);star.textContent='★ Saved';};
    const after=document.createElement('button');after.type='button';after.className='study-question-star study-after-answer';after.textContent='☆ Save this question';after.hidden=true;after.onclick=()=>star.click();
    const mount=()=>{const actions=document.querySelector('.practice-actions,.answer-actions,.qa-actions')||document.querySelector('#submitBtn')?.parentElement;if(actions&&!star.isConnected){actions.append(star,after);} };
    const timer=setInterval(()=>{const app=window.SpanishPracticeApp;if(!app)return;clearInterval(timer);mount();const syncStars=(q)=>{const isSaved=data.saved.some(x=>x.key===`${q?.module}:${q?.id}`);star.textContent=isSaved?'★ Saved':'☆ Save question';star.disabled=isSaved;star.hidden=!!app.answered&&!isSaved;after.hidden=!app.answered||isSaved;};const orig=app.renderQuestion?.bind(app);if(orig)app.renderQuestion=(q,o)=>{const out=orig(q,o);mount();syncStars(q);return out;};setInterval(()=>{if(app.currentQuestion)syncStars(app.currentQuestion);},200);review.querySelector('.study-review-start').onclick=()=>{const selected=data.saved.filter(q=>q.selected!==false);if(!selected.length)return;review.hidden=true;const app=window.SpanishPracticeApp;if(!app)return;app.__reviewQueue=[...selected];app.__reviewLast=false;if(!app.__baseNextQuestion)app.__baseNextQuestion=app.nextQuestion.bind(app);app.__reviewNext=app.__baseNextQuestion;app.nextQuestion=function(opts={}){if(this.__reviewLast){if(this.currentQuestion&&!this.canAdvanceFromCurrentQuestion())return this.__reviewNext(opts);this.__reviewLast=false;this.__reviewQueue=null;this.finishPracticeSession();this.showBanner?.('My Review session complete.');return;}if(this.__reviewQueue?.length){const item=this.__reviewQueue.shift();if(!this.__reviewQueue.length)this.__reviewLast=true;const q={...item,id:item.qid||item.key,module:item.module,mode:item.mode||(item.module==='numbers'?'numbers':'text'),prompt:item.prompt,expectedDisplay:item.answer,number:item.number,sp:item.answer,acceptable:new Set(item.acceptable||[]),options:item.options,correctIndex:item.correctIndex,explanation:item.explanation};this.currentQuestion=q;this.currentNumber=q.number??this.currentNumber;this.answered=false;this.sessionQuestionRecorded=false;this.prepareAnswerTarget?.(q);this.updateActiveModuleChip?.(q.module);this.renderQuestion(q);return;}this.__reviewQueue=null;this.__reviewNext(opts);};if(document.body.classList.contains('home-mode')){app.$.homeCard.style.display='none';app.$.mainCard.style.display='block';document.body.classList.remove('home-mode');document.body.classList.add('practice-mode');app.startPracticeSession();app.currentQuestion=null;app.nextQuestion();}else {app.currentQuestion=null;app.nextQuestion();}};},50);
  };
